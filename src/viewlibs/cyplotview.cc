 ...
 * @LastEditTime: 2025-10-23 14:37:40
 * @FilePath: /t5_ai_coffee/src/viewlibs/cyplotview.cc
 * @Description: 
 * @BugList: 
 * 
...
DECLARE_WIDGET(CYPlotView)

class CYPlotView::Private
{
public:
    Private(CYPlotView *qq)
        : q(qq)
    ...
    // List of PlotObjects
    std::vector<CYPlotObject *> objectList;
...
CYPlotView::CYPlotView(int w,int h):View(w,h),d(new Private(this)){
...
CYPlotView::CYPlotView(cdroid::Context*ctx,const cdroid::AttributeSet&atts)
...
CYPlotView::~CYPlotView()
{
    delete d;
}

/*QSize CYPlotView::minimumSizeHint() const
{
    return QSize(150, 150);
}

QSize CYPlotView::sizeHint() const
{
    return size();
}*/

void CYPlotView::setLimits(double x1, double x2, double y1, double y2)
{
    d->calcDataRectLimits(x1, x2, y1, y2);
    invalidate();
}

void CYPlotView::Private::calcDataRectLimits(double x1, double x2, double y1, double y2)
{
    double XA1;
    double XA2;
    double YA1;
    ...
    LOGI("q = %p  BottomAxis = %d  LeftAxis = %d",q,q->axis(BottomAxis)->majorTickMarks().size(),q->axis(LeftAxis)->majorTickMarks().size());
}

void CYPlotView::setSecondaryLimits(double x1, double x2, double y1, double y2)
{
    double XA1;
    double XA2;
    double YA1;
...
void CYPlotView::clearSecondaryLimits(){
...
RectF CYPlotView::dataRect() const{
    return d->dataRect;
}

RectF CYPlotView::secondaryDataRect() const{
    return d->secondDataRect;
}

void CYPlotView::addPlotObject(CYPlotObject *object){
    // skip null pointers
    if (!object) {
        return;
    }
...
void CYPlotView::addPlotObjects(const std::list<CYPlotObject *> &objects){
    bool addedsome = false;
    for (CYPlotObject *o : objects) {
        if (!o) {
            continue;
        }
...
std::vector<CYPlotObject *> CYPlotView::plotObjects() const{
    return d->objectList;
}

void CYPlotView::setAutoDeletePlotObjects(bool autoDelete){
    d->autoDelete = autoDelete;
}

void CYPlotView::removeAllPlotObjects(){
...
void CYPlotView::resetPlot(){
    ...
    a->setLabel(std::string());
    a->setTickLabelsShown(false);
    axis(CYPlotView::LeftAxis)->setLabel(std::string());
    axis(CYPlotView::BottomAxis)->setLabel(std::string());
    resetPlotMask();
}

void CYPlotView::replacePlotObject(int i, CYPlotObject *o){
...
uint32_t CYPlotView::gridColor() const{
    return d->cGrid;
}

void CYPlotView::setGridColor(const uint32_t &gc){
    d->cGrid = gc;
    invalidate();
}


uint32_t CYPlotView::foregroundColor()const{
    return d->cForeground;
}

void CYPlotView::setForegroundColor(const uint32_t fg){
    d->cForeground = fg;
    invalidate();
}

bool CYPlotView::isGridShown() const{
    return d->showGrid;
}

bool CYPlotView::isObjectToolTipShown() const{
    return d->showObjectToolTip;
}

bool CYPlotView::antialiasing() const{
    return d->useAntialias;
}

void CYPlotView::setAntialiasing(bool b){
    d->useAntialias = b;
    invalidate();
}

void CYPlotView::setShowGrid(bool show){
    d->showGrid = show;
    invalidate();
}

void CYPlotView::setObjectToolTipShown(bool show){
    d->showObjectToolTip = show;
}

PlotAxis *CYPlotView::axis(Axis type){
    auto it = d->axes.find(type);
    return it != d->axes.end() ? it->second : nullptr;
}

const PlotAxis *CYPlotView::axis(Axis type) const{
    auto it = d->axes.find(type);
    return it != d->axes.end() ? it->second : nullptr;
}

Rect CYPlotView::pixRect() const{
    return d->pixRect;
}

std::list<PlotPoint *> CYPlotView::pointsUnderPoint(const Point &p) const
{
    std::list<PlotPoint *> pts;
    for (const CYPlotObject *po : d->objectList) {
...
/*bool CYPlotView::event(QEvent *e){
...
void CYPlotView::onSizeChanged(int w,int h,int oldw,int oldh){
    View::onSizeChanged(w,h,oldw,oldh);
    setPixRect();
    resetPlotMask();
}

void CYPlotView::setPixRect(){
...
PointF CYPlotView::mapToWidget(const PointF &p) const{
...
void CYPlotView::maskRect(const RectF &rf, float fvalue){
...
void CYPlotView::maskAlongLine(const PointF &p1, const PointF &p2, float fvalue){
...
void CYPlotView::placeLabel(cdroid::Canvas&painter, PlotPoint *pp){
    ...
    // 增加一点点间距，更美观一点（否则该文本会与线条、柱形图顶部有些许重叠）
    Rect rctxt = Rect::Make(bestRect.left,bestRect.top+5,bestRect.width,bestRect.height);
...
float CYPlotView::Private::rectCost(const RectF &r) const{
...
void CYPlotView::onDraw(cdroid::Canvas&p){
    ...
    for (CYPlotObject *po : d->objectList) {
        po->draw(p, this);
    }
...
void CYPlotView::drawAxes(cdroid::Canvas&p){
    if (d->showGrid) {
    ...
    bool visible = a->isVisible();
    if (visible || a->areTickLabelsShown()) {
        // Draw axis line
        if(visible){
        ...
        for (const double xx : majMarks) {
            ...
            if (px > 0 && px <= d->pixRect.width) {
                ...
                if (a->areTickLabelsShown()) {
                    ...
                    // LOGE("a->tickLabel(xx) = %s  (%d,%d,%d,%d) a->label() = %s  %d",a->tickLabel(xx).c_str(),-2 * BIGTICKSIZE - SMALLTICKSIZE, int(px) - SMALLTICKSIZE, 2 * BIGTICKSIZE, 2 * SMALLTICKSIZE,a->label().c_str(),xx == majMarks.back());
                    bool isShowLabel = !a->label().empty() && (xx == majMarks.back());
                    p.draw_text(r,isShowLabel?a->tickLabel(xx)+" ("+a->label()+")":a->tickLabel(xx),cdroid::Gravity::CENTER);
                }
            }
        }
    ...
    if (visible || a->areTickLabelsShown()) {
        ...
        for (const double yy : majMarks) {
            ...
            if (py >= 0 && py <= d->pixRect.height) {
                ...
                if (a->areTickLabelsShown()) {
                    ...
                    LOGE("a->tickLabel(xx) = %s  (%d,%d,%d,%d)",a->tickLabel(yy).c_str(),-2 * BIGTICKSIZE - SMALLTICKSIZE, int(py) - SMALLTICKSIZE, 2 * BIGTICKSIZE, 2 * SMALLTICKSIZE);
                    bool isShowLabel = !a->label().empty() && (yy == majMarks.back());
                    p.draw_text(r, isShowLabel?"("+a->label()+") "+a->tickLabel(yy):a->tickLabel(yy),Gravity::RIGHT|Gravity::CENTER_VERTICAL);
                }
            }
        }
        ...
        //     p.translate(-XPADDING,0);
        //     p.rotate_degrees(-90.0);

        //     Rect r = {0, 0, XPADDING, XPADDING};
