 ...
 */
CHAR_T *sf_pack_local_obj_dp_data(IN DEV_CNTL_N_S *dev_cntl, IN CONST UINT_T max_len, IN CONST BOOL_T addDevId, OUT BOOL_T *p_all_data_packed, BOOL_T reset_flow_ctl, IN CONST BOOL_T is_lan, IN BOOL_T ble_rept, OUT CHAR_T** time_str);
...
typedef struct {
    ...
    DP_REPT_FORM_T         rept_form;   // report form, 0:active, 1:query, 2:sync
    BYTE_T                 is_ble_roam; // ble roam flag, 0: no, 1:yes
    UINT_T                 rept_seq;    // report sequence for ble roam
    //TY_DP_REPT_PARAM_ASYNC rept_param;  // report param
} TY_OBJ_DP_REPT_S;
...
typedef struct {
    ...
    BYTE_T                is_ble_roam;    // ble roam flag, 0: no, 1:yes
    UINT_T                rept_seq;       // report sequence for ble roam
    //TY_DP_REPT_PARAM_SYNC rept_param;     // report param
} TY_STAT_DP_REPT_S;
...
/**
 * @brief set dp id report rule
 *
 * @param[in] rept_rule: ref to DP_ID_REPT_TYPE_E
 *
 * @note This API is used to set dp id report rule
 *
 * @return VOID
 */
...
/**
 * @brief get dp id report rule
 *
 * @return dp id rept vaule
 */
UINT8_T ty_get_dpid_rept_rule(VOID_T);

/**
 * @brief check dp id need report
 *
 * @param[in] dev_id: dev id
 * @param[in] dpid: dp id
 * @param[in] type: ref type DP_CMD_TYPE_E
 * @param[in] DP_DESC_IF_S: dp desc info
 *
 * @note This API is used to check if dp id need rept
 *
 * @return true:need rept,false:no need rept
 */
BOOL_T ty_check_dpid_need_rept(CHAR_T *dev_id, BYTE_T dpid, DP_CMD_TYPE_E type, DP_DESC_IF_S *dp_desc);

#if defined(ENABLE_COMMUNICATE_PRIORITY) && (ENABLE_COMMUNICATE_PRIORITY==1)
/**
...
/**
 * @brief Dp report or ble sub device
 *
 * @param[in] data: dp data
 * @param[in] len: lenth of dp data
 * @param[in] to_lmt: timeout for reporting
 * @param[in] prv_data: smart point
 *
 * @note This API is used for reporting dp for ble sub device. this API should be called after device initialization.
 *
 * @return OPRT_OK on success. Others on error, please refer to tuya_error_code.h
 */
OPERATE_RET sf_ble_sub_dev_dp_report_async(IN CONST CHAR_T *data, IN CONST INT_T len, IN CONST UINT_T to_lmt, IN VOID *prv_data);
#endif
