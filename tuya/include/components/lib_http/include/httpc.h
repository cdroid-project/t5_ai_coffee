...
int http_open_session_with_config(http_session_t *handle, const char *hostname, \
                                  tuya_tls_config_t* tls_config, int flags, int retry_cnt);
 ...
 * @param[in] handle Handle returned from the call to http_new_session()
 *
 * @return TRUE if the given session handle is closed, FALSE otherwise
 */
...
/**
 * @brief This API is used to SET HTTP recv timeout
 *
 * @param[in] timeout_s timeout (unit:s)
 *
 * @return OPRT_OK on success. Others on error, please refer to tuya_error_code.h
 */
...
/**
 * @brief This API is used to GET HTTP recv timeout
 *
 * @param[in]
 *
 * @return count of http redirect
