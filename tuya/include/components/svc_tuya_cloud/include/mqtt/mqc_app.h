 ...
 *
 * @param[in] p_msg Data
 * @param[in] msg_len Data length
 * @param[in] timeout Timeout to get ACK or else report error
 *
 * @return OPRT_OK on success. Others on error, please refer to tuya_error_code.h
 */
OPERATE_RET mqc_media_pub_sync_v3(BYTE_T *p_msg, UINT_T msg_len, UINT_T timeout);
...
/**
 * @brief pack flow data
 *
 * @param[in] dt_body flow data
 * @param[out] out_data out data
 * @param[out] out_len out data length
 *
 * @return OPRT_OK on success. Others on error, please refer to tuya_error_code.h
 */
OPERATE_RET mqc_pack_flow_data(IN CONST FLOW_BODY_ST *dt_body, OUT BYTE_T **out_data, OUT UINT_T *out_len);
