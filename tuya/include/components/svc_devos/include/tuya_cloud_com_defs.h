...
#define BEACON_KEY_LEN          16      // max string length of BEACON_KEY
...
#define DEV_MATTER_SNGL     6   //Thread
...
/*局域网升级，app发送升级包到wifi设备*/
#define UPGRADE_TYPE_LAN    2
...
typedef struct {
    ...
    /** serialize data len */
    UINT_T len;
    /** serialize buf, malloc when schema parse, default was 0 */
    VOID *data;
    /** deserialize buf*/
    CHAR_T *deserialize;
} DP_PROP_STRUCT_S;
...
typedef struct {
    ...
    /** serialize data len */
    INT_T len;
    /** serialize buf, malloc when dp report, default was 0 */
    VOID *data;
    /** deserialize buf*/
    CHAR_T *deserialize;
} DP_PROP_ARRAY_S;
...
typedef union {
    ...
    /** see DP_PROP_STRUCT_S */
    DP_PROP_STRUCT_S prop_struct;
    /** see DP_PROP_ARRAY_S */
    DP_PROP_ARRAY_S prop_array;
} DP_PROP_VALUE_U;
...
#define DP_CMD_MQ_DAEMON   8    // cmd from MQTT(daemon)
#define DP_CMD_EXT_APP      100     // 应用扩展预留范围[100,110],避免冲突
...
/**
 * @brief Definition of ctrl struct and array dp
 */
typedef struct {
    /** data len */
    UINT_T len;
    /** data buf */
    CHAR_T *data;
} TY_OBJ_STRUCT_S, TY_OBJ_ARRAY_S;
...
typedef union {
    ...
    TY_OBJ_STRUCT_S dp_struct;  // valid when dp type is struct
    TY_OBJ_ARRAY_S dp_array;    // valid when dp type is array
} TY_OBJ_DP_VALUE_U;
...
/**
 * @brief tuya sdk dp report form
 */
typedef BYTE_T DP_REPT_FORM_T;
#define DP_REPT_FORM_ACTIVE     0       // active report
#define DP_REPT_FORM_QUERY      1       // query report
#define DP_REPT_FORM_SYNC       2       // sync report

typedef struct {
    /** report from, 0:active, 1:query, 2:sync */
    DP_REPT_FORM_T rept_form;
    /** report force or not */
    BYTE_T force;
    /** ble roam flag, 0: no, 1:yes */
    BYTE_T is_ble_roam;
    /** report sequence for ble roam */
    UINT_T rept_seq;
...
typedef struct {
    ...
    /** timestamp in string format for raw dp*/
    CHAR_T *time_str;
    /** ble roam flag, 0: no, 1:yes */
    BYTE_T is_ble_roam;
    /** report sequence for ble roam */
...
/**
 * @brief Definition of DP filerting param
 */
typedef struct {
    /** dp id */
    UINT_T        dp_id;
    /** source of dp command */
...
/**
 * @brief Handler of dp receiving filterging
 *
 * @param[in] dp_info informationg of dp, see DP_RECV_FILTER_PARA
 *
 * @return TRUE on filtered. Others FALSE
 */
typedef BOOL_T(*DP_RECV_FILTER_HANDLE)(IN CONST DP_RECV_FILTER_PARA* dp_info);
...
/**
 * @brief Handler of dp async report
 *
 * @param[in] rslt Report result
 * @param[in] data DP data in cjson format
 *
 * @return OPRT_OK on success. Others on error, please refer to tuya_error_code.h
 */
typedef OPERATE_RET(*RAW_DP_ASYNC_REPT_CB)(OPERATE_RET rslt, CHAR_T* data);

/**
 * @brief Definition of DP handlers
*/
typedef struct {
    /** handler of receiving dp filtering */
 ...
 * @param[in] pri_data Private data
 */
typedef OPERATE_RET(*UPGRADE_NOTIFY_CB)(IN CONST FW_UG_S *fw, IN CONST INT_T download_result, IN PVOID_T pri_data);
...
typedef BYTE_T DEV_EXT_MODE_TYPE;
#define DEV_EXT_MODE_MAIN       0
#define DEV_EXT_MODE_NODEB      1
#define DEV_EXT_MODE_WIFI       2
#define DEV_EXT_MODE_CAT1       3

/* Information of ext module */
typedef struct {
    DEV_EXT_MODE_TYPE emt;
    CHAR_T uni_code[20];  //uni code of ext module, wifi:mac(len:6), cat1:imei(len:18)
} DEV_EXT_MODULE_INFO;
